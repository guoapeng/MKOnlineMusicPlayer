...
DataFetcher.prototype = {
    ...
    ajaxUrl: function (music, callback)
    {
        // 已经有数据，直接回调
    ...
    // 完善获取音乐封面图
    // 包含音乐信息的数组、回调函数
    ajaxPic: function (music, callback)
    {
        // 已经有数据，直接回调
    ...
    ajaxPlayList: function (lid, id, callback) {
        if(!lid) return false;
            ...
            success: function(jsonData){
                ...
                if(musicList[id].creatorID) {
                    ...
                    if(musicList[id].creatorID === rem.uid) {   // 是当前登录用户的歌单，要保存到缓存中
                        var tmpUlist = rem.dataSaver.playerReaddata('ulist');    // 读取本地记录的用户歌单
                        if(tmpUlist) {  // 读取到了
                            for(i=0; i<tmpUlist.length; i++) {  // 匹配歌单
                                if(tmpUlist[i].id == lid) {
                                    tmpUlist[i] = tempList; // 保存歌单中的歌曲
                                    rem.dataSaver.playerSavedata('ulist', tmpUlist);  // 保存
                                    break;
                                }
                            }
                        }
    ...
    // ajax加载歌词
    // 参数：音乐ID，回调函数
    ajaxLyric: function (music, callback) {
        lyricTip('歌词加载中...');
        ...
        $.ajax({
            ...
            success: function(jsonData){
                ...
                if(jsonData.playlist.length === 0 || typeof(jsonData.playlist.length) === "undefined")
                {
                    ...
                    // 记录登录用户
                    rem.dataSaver.playerSavedata('uid', rem.uid);
                    rem.dataSaver.playerSavedata('uname', rem.uname);
                    ...
                    rem.dataSaver.playerSavedata('ulist', userList);
                    // 显示退出登录的提示条
